...
# Confronta a schermo TUTTE le Varianti del Progetto (fino a 9 Approcci):
...
#   5b. Agente Neurale Focal Loss Neurosimbolica Completa -> per_zone_checkpoint_focal_completa.pth
#   6. Agente Neurale Contesto Esterno (Ring Semantics) -> per_zone_checkpoint_surrounding.pth
#   7. Agente Neurale Asymmetric Loss + Penalizzazione -> per_zone_checkpoint_asl.pth
#   8. Agente Neurale Asymmetric Loss Standard -> per_zone_checkpoint_asl_standard.pth
#   8b. Agente Neurale Asymmetric Loss Neurosimbolica Completa -> per_zone_checkpoint_asl_completa.pth
#   9. Agente Neurale Temporale (Ramp-Up/Decay Bayesiano) -> extracted_occlusions_temporal/

# Import dei moduli di sistema per la gestione dei percorsi di ambiente
import os
import sys
# Import di glob per reperire i file di predizione salvati su disco
import glob
# Import di json per la lettura e deserializzazione dei file di output
import json
# Import di numpy per operazioni di calcolo algebrico ed array multidimensionali
import numpy as np
# Import di torch per l'inferenza accelerata tramite GPU o CPU
import torch
# Import di DataLoader per il caricamento efficiente a batch dei dati
from torch.utils.data import DataLoader

# Aggiunge la directory radice del progetto al sys.path per consentire l'importazione dei moduli accessori
...
def run_comparative_evaluation(threshold=0.30):
    # Stampa dell'intestazione principale della valutazione comparativa complessiva
    print("\n" + "=" * 95)
    ...
    # Caricamento dell'intero dataset nuScenes (18.682 coni d'ombra)
    ...
    # Funzione ausiliaria per istanziare un modello neurale e caricarne i pesi addestrati
    def load_model(ckpt_path):
        ...
        if os.path.exists(full_path):
            try:
                # Carica lo stato dei pesi salvati nel file di checkpoint PyTorch
    ...
    # Caricamento dei checkpoint dei vari modelli neurali addestrati presenti su disco
    ...
    # Cache Agente Temporale: legge i file JSON delle probabilità con filtro temporale pre-calcolato
    ...
    # Inizializzazione dei contatori TP, FP e FN per tutte le 9 varianti sui 6 canali di classe
    ...
    # Contatori per il numero di violazioni della coerenza semantica (veicoli su terreno/marciapiede)
    ...
    def get_model_preds(model, has_model):
        if not has_model:
            return None
        model.eval()
        # Istanzia un DataLoaderPyTorch a batch size 256 per velocizzare l'inferenza
    ...
    # Esegue il pre-calcolo parallelo su GPU per ciascuno dei modelli neurali disponibili
    ...
    # Scorre tutti i campioni dell'intero dataset nuScenes
    with torch.no_grad():
        for idx, sample in enumerate(ds.samples):
            # Vettore di Ground Truth reale
            target_vec = sample['target'].numpy()
            sample_token = sample['sample_token']
            raw_pts = sample['polygon_points']
            
            sc_np = sample['scalars'].numpy()
            # Identifica se il cono d'ombra ricade su marciapiede (indice 5) o terreno (indice 8)
            is_non_driveable = (sc_np[5] + sc_np[8]) > 0.5
            if is_non_driveable:
                non_driveable_zones += 1

            # Funzione interna per valutare le predizioni di un modello e calcolare TP, FP, FN e violazioni
            ...
            # Esegue la valutazione per tutti i modelli presenti
            ...
            # Valutazione Agente Temporale: legge la prob temporale pre-calcolata per sample_token + centroide
            if has_temporal:
    ...
    # Funzione interna per la stampa elegante della tabella delle metriche
    ...
    # Stampa a schermo delle tabelle per ciascuna delle varianti disponibili
...
# Blocco principale di esecuzione se avviato direttamente da riga di comando
