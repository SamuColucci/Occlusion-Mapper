# PerZoneModel per la predizione delle occlusioni per-zone in coordinate BEV (per_zone_model.py)
# Rete neurale ibrida (CNN + MLP) con 11 canali d'ingresso BEV + 9 feature scalari numeriche e 6 canali di output
# Ingresso: 
#   Patch Visivo 2D (11, 64, 64):
...
#     Canale 7: Oggetti visibili Pedone
#     Canale 8: Oggetti visibili Moto
#     Canale 9: Oggetti visibili Bici
#     Canale 10: Oggetti visibili Barriera/Struttura
#   Feature Scalari Numeriche (9,):
...
#     Scalare 4-8: Frazioni di terreno One-Hot (Strada, Marciapiede, Strisce, Parcheggio, Terreno)
# Output: 
#   Vettore di 6 Logits / Probabilità Sigmoid:
#     Canale 0: Probabilità di Auto occlusa
#     Canale 1: Probabilità di Camion/Bus occluso
#     Canale 2: Probabilità di Pedone occluso
#     Canale 3: Probabilità di Moto occlusa
#     Canale 4: Probabilità di Bicicletta occlusa
#     Canale 5: Probabilità di Barriera occlusa

# Import di torch per le operazioni tensoriali PyTorch
import torch
# Import di nn (Neural Network Module) per la costruzione dei layer della rete neurale
import torch.nn as nn
# Import di F per le funzioni di attivazione e trasformazione funzionali
...
class PerZoneModel(nn.Module):
    # in_channels: numero di canali d'ingresso del patch visivo (11 canali BEV)
    # num_scalars: numero di feature numeriche scalari dell'ombra (9 scalari: 4 geometrici + 5 One-Hot semantici del terreno)
    # num_classes: numero di classi target d'uscita (6 classi semantiche)
    def __init__(self, in_channels=11, num_scalars=9, num_classes=6):
        # Invocazione del costruttore della classe padre nn.Module
        ...
        # da 11 a 128, fino ad estrarre un vettore compatto di 512 caratteristiche visive ad alto livello della zona d'ombra
        self.conv_net = nn.Sequential(
            # Conv2d (Convolutional 2D Layer): applica filtri convoluzionali 2D per estrarre caratteristiche di contorno e forma (11 canali -> 32 filtri)
            ...
            # BatchNorm2d (Batch Normalization 2D): normalizzazione del secondo stadio
            nn.BatchNorm2d(64),
            ...
            # BatchNorm2d (Batch Normalization 2D): normalizzazione del terzo stadio
            nn.BatchNorm2d(128),
            # ReLU (Rectified Linear Unit): attivazione non lineare
            nn.ReLU(inplace=True),
        ...
        # Classificatore Ibrido Multimodale MLP (256 Feature Visive + 9 Feature Scalari Numeriche = 265 Input Totali)
        self.classifier = nn.Sequential(
            # Linear Layer: concatena feature visive e scalari numeriche (265 -> 128 neuroni)
            ...
            nn.Dropout(0.2),
            # Linear Layer finale: produce i 6 Logits grezzi di output per ciascuna classe semantica
            nn.Linear(128, num_classes)
        )

    # Il metodo forward definisce la pipeline d'inferenza:
    # La CNN estrae 512 feature visive dal patch 2D che vengono proiettate a 256 e concatenate con i 9 scalari dell'ombra
    # Il vettore multimodale risultante di 265 elementi viene infine elaborato dal classificatore MLP per produrre i 6 logits di presenza per classe
    def forward(self, patch_tensor, scalar_features):
        ...
        # Proiezione lineare e attivazione ReLU (256 feature)
        x_vis = F.relu(self.fc_vis(x_vis))
        
        # Concatenazione delle 256 feature visive con i 9 scalari numerici (265 feature totali)
        x_combined = torch.cat([x_vis, scalar_features], dim=1)
        
        # Classificazione finale ed estrazione dei 6 Logits di output
...
if __name__ == "__main__":
    ...
    model = PerZoneModel()
    dummy_patch = torch.randn(4, 11, 64, 64)
    dummy_scalars = torch.randn(4, 9)
