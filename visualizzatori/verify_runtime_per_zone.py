...
# Classe principale per la gestione del visualizzatore interattivo dell'Agente Neurale Per-Zone
class PerZoneRuntimeVisualizer:
    def __init__(self, mode="per_zone"):
        # Imposta il nome ed il percorso sorgente predefinito per le predizioni per-zone
        self.modo_label = "NEURALE PER-ZONE (Patch 64x64 + Scalari)"
        self.in_dir = "extracted_occlusions_per_zone"
        # Se la cartella principale per-zone non esiste o è vuota, utilizza la cartella neurale di fallback
        ...
        # Recupera ed ordina tutti i file JSON delle predizioni per-zone presenti nella cartella
    ...
    def load_json_file(self):
        # Recupera il percorso completo del file JSON corrente
        json_path = self.json_files[self.current_file_idx]
        # Apre e deserializza il file JSON dell'Agente Per-Zone
    ...
    def plot_current(self):
        ...
        # 2. Disegno di tutte le altre occlusioni NON selezionate (zorder=4) in azzurro/blu trasparente
        for idx, occ in enumerate(self.occlusions):
            ...
            if len(poly_pts) >= 3:
                ...
                # Calcola il baricentro dell'occlusione secondaria per posizionarvi l'etichetta dell'indice
        ...
        # Estrae i dati dell'occlusione attiva selezionata
        ...
        # 3. Disegno dell'occlusione selezionata in rosso scuro/rosa acceso
        ...
        for sz in sub_zones:
            ...
            if len(sz_pts) >= 3:
                ...
                # Calcola il centro della sotto-zona e stampa la percentuale di superficie occupata
        ...
        # Stampa l'etichetta dell'indice (#1, #2) al centro dell'occlusione selezionata
        if len(curr_poly_pts) >= 3:
        ...
        # Formattazione dell'intestazione superiore della mappa
        ...
        # Mostra la ripartizione percentuale delle sotto-zone
        ...
        # Categoria principale predetta dalla Rete Per-Zone
        ...
        # Se sono presenti le sotto-zone, impagina le probabilità dettagliate per ciascuna sotto-zona
        if sub_zones:
            hud_lines.append("=" * 30)
        ...
        # Posiziona la legenda del grafico a destra della mappa
...
if __name__ == "__main__":
    # Istanzia ed avvia il visualizzatore dell'Agente Per-Zone
