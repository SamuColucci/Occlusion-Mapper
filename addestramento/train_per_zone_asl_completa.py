# Script di Addestramento Asymmetric Loss Neurosimbolica Completa (train_per_zone_asl_completa.py)
# Addestra il modello PerZoneModel con AsymmetricLoss_neurosimbolica_completa (20 Epoche con Cosine Scheduler)
# Salva il checkpoint dei pesi addestrati nel file per_zone_checkpoint_asl_completa.pth
...
# Importazione dell'architettura neurale PerZoneModel dal pacchetto dedicato
from architettura_neurale.per_zone_model import PerZoneModel
# Importazione del caricatore del dataset OcclusionDatasetPerZone dal pacchetto adapter
from dataset_adapter.dataset_generator_per_zone import OcclusionDatasetPerZone
# Importazione della funzione di perdita Asymmetric Loss Neurosimbolica Completa
from architettura_neurale.loss_functions import AsymmetricLoss_neurosimbolica_completa

def train_asl_completa_model(epochs=20, batch_size=32, lr=1e-3, checkpoint_path=os.path.join("pesi_modelli", "per_zone_checkpoint_asl_completa.pth")):
    # Assicura la presenza della cartella pesi_modelli su disco
    os.makedirs(os.path.dirname(checkpoint_path), exist_ok=True)
    print("\n" + "=" * 85)
    print("   ADDESTRAMENTO VARIANTE: ASYMMETRIC LOSS NEUROSIMBOLICA COMPLETA (INIBIZIONE + PROMOZIONE VRU - 20 EPOCHE)")
    print("=" * 85)
    ...
    # Loss ASL Neurosimbolica Completa ed Ottimizzatore AdamW con Scheduler Coseno
    criterion = AsymmetricLoss_neurosimbolica_completa(gamma_neg=4.0, gamma_pos=1.0, clip=0.05)
    optimizer = torch.optim.AdamW(model.parameters(), lr=lr, weight_decay=1e-4)
    scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=epochs, eta_min=1e-5)

    # Ciclo di Addestramento per Epoche
    print(f"\nInizio Addestramento Asymmetric Loss Completa per {epochs} Epoche...\n")
    ...
    for epoch in range(epochs):
        ...
        print(f"  [Epoca {epoch+1:2d}/{epochs:2d}] Loss Media ASL Completa: {avg_loss:.4f} | LR: {current_lr:.6f}")

    total_time = time.time() - start_time
    print(f"\nAddestramento Asymmetric Loss Completa terminato in {total_time:.2f} secondi.")
...
if __name__ == "__main__":
    train_asl_completa_model(epochs=20, batch_size=32, lr=1e-3)
