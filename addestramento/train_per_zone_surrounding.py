...
# Addestra il modello PerZoneModel sul contesto semantico dell'Anello Esterno Circostante (Surrounding Ring) per lo studio di ablazione
...
# Import del caricatore del dataset OcclusionDatasetSurrounding dal pacchetto adapter
from dataset_adapter.dataset_generator_surrounding import OcclusionDatasetSurrounding
# Import della funzione di perdita Focal Loss con penalizzazione semantica
...
def train_surrounding_model(epochs=20, batch_size=32, lr=1e-3, checkpoint_path=os.path.join("pesi_modelli", "per_zone_checkpoint_surrounding.pth")):
    ...
    # Caricamento del Dataset basato sull'Anello Semantico Circostante
